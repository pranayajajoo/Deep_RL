...
if __name__ == '__main__':
    ...
    parser.add_argument("--hyper-to-ablate", type=str, default="target_update_interval")
    ...
    if args.hyper_to_ablate == "target_update_interval":
        hyper_to_ablate = [1, 10, 100]
        buffer_size = 25000
        iter_colors = ['r', 'b', 'green']
    else:
        args.hyper_to_ablate = "buffer_size"
        hyper_to_ablate = [100, 500, 5000, 25000]
        target_update_interval = 100
        iter_colors = ['r', 'b', 'green', 'black']

    perf_dict = {hyper: {} for hyper in hyper_to_ablate}
    q_val_dict = {hyper: {} for hyper in hyper_to_ablate}

    for hyper in hyper_to_ablate:
        for agent_class in agent_classes:
            ...
            for seed in range(num_seeds):
                ...
                if args.hyper_to_ablate == "target_update_interval":
                    ...
                    buffer = replay_buffer.ReplayBuffer(hyper, discount=discount, n_step=n_step)

                if args.hyper_to_ablate == "target_update_interval":
                    agent = dqn.DQN(q_network, optimizer, buffer, explorer, discount, hyper,
                ...
                episode_returns, q_values = agent_environment.agent_environment_episode_loop(
                    agent, env, num_training_episodes, args.debug, args.track_q
                )
                alg_returns.append(episode_returns)
                alg_q_values.append(q_values)

            perf_dict[hyper][agent_text] = alg_returns
            q_val_dict[hyper][agent_text] = alg_q_values

            plot_alg_results(perf_dict[hyper][agent_text], f"{agent_text}_{hyper}_{args.hyper_to_ablate}_cartpole.png", label=agent_text)
            if args.track_q:
                plot_alg_results(q_val_dict[hyper][agent_text], f"{agent_text}_{hyper}_{args.hyper_to_ablate}_cartpole_q_vals.png",
                                label=agent_text, ylabel="Q-values", title="Q-values")

    plot_many_algs(
        [perf_dict[it][agent_text] for it in hyper_to_ablate],
        [f"{it}_{args.hyper_to_ablate}_{agent_text}" for it in hyper_to_ablate],
        iter_colors,
        f"{agent_text}_{args.hyper_to_ablate}_cartpole.png"
    )